...
export function CustomizerBeforeRefactor({ onAddToCart }: CustomizerProps) {
  // ── Customizer state ───────────────────────────────────────────────────────
  const {
    activeDesign,
    setActiveDesign,
  ...
  const handleBuyNow = () => {
    ...
    const hash = btoa(
      unescape(encodeURIComponent(JSON.stringify(customization))),
    ).substring(0, 8);
            ...
            <>
              ...
              <div>
                ...
                <p className="font-mono text-xs text-stone-400 mb-3">
                  {SHIRT_COLORS.find((c) => c.hex === shirtColor)?.name ??
                    "Snow White"}
                </p>
                <ColorPicker
                  colors={SHIRT_COLORS}
